...
class MainActivity : AppCompatActivity() {
    ...
    private fun isServicesOK() : Boolean {
        Log.d(MainActivity.TAG,"::: isServicesOK() : Checking Google Play Services version... :::")
        ...
        if (available == ConnectionResult.SUCCESS) {
            ...
            Log.d(MainActivity.TAG, "::: isServicesOK() : Google Play Services is working! :::")
            return true
        } else if (GoogleApiAvailability.getInstance().isUserResolvableError(available)) {
            // An error occured but we can resolve it
            Log.d(MainActivity.TAG, "::: isServicesOK() : An error occured. But we can fix it! :::")
            // Ask Google to give a dialog, where we can find a solution to the problem
            val dialog = GoogleApiAvailability.getInstance().getErrorDialog(this, available, MainActivity.ERROR_DIALOG_REQUEST)
            dialog.show()
        } else {
